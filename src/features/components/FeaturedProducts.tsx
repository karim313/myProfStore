...
const normalizeProduct = (p: any): any => {
  ...
  return {
    ...p,
    displayTitle: p.name || p.title || '',
    displayImage: getPrimaryImage(p) || '',
    displayPrice: p.finalPrice || p.price || 0,
    displayOriginalPrice: p.originalPrice || 0,
    displayBadge: p.offerEndDate ? `ينتهي: ${p.offerEndDate}` : undefined,
    isFeatured: p.discountPercentage ? p.discountPercentage > 0 : (p.id % 3 === 0),
  }
}
...
function ProductCard({ product, onSelect, size = 'small' }: ProductCardProps) {
  ...
  return (
    <button
      ...
      aria-label={`${norm.displayTitle} — ${norm.displayPrice.toLocaleString()} د.ع`}
