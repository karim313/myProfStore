...
// ─── Component ────────────────────────────────────────────────────────────────
export default function CategorySlider({ products }: { products: Product[] }) {
  ...
  const coverCategories = products.filter(
    (product, index, array) =>
      product.category && index === array.findIndex(item => item.category === product.category)
  )
          ...
          {coverCategories.map(cat => (
            ...
            >
              ...
              <span className='text-center text-lg font-semibold absolute bottom-2 right-2 bg-gray-900/40 px-4 py-2 rounded-lg text-gray-200'>
                {arabicCategoryMap[cat.category] || cat.category}
              </span>
            </div>
          ))}
        </div>
